...
export default function Buttons() {
    const {increment, decrement, questionIndex, userInput, loading} = useContext(SurveyContext)
    if(loading === false){
    return (
        <>
        <p className="mt-4 pl-md-5 pr-4 pr-md-5">
            ...
            <span className="back-next-controls">
                ...
                <button className="nextBtn btn btn-light" onClick={increment} disabled={userInput.length-1 < questionIndex}>Next &nbsp;<i className="fal fa-arrow-right"></i></button>
            </span>
        </p>
        </>
    )
